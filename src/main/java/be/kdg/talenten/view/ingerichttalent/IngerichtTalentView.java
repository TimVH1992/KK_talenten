...
public class IngerichtTalentView extends BorderPane {
    ...
    public IngerichtTalentView() {
        sidebar = new AppSidebar(AppSidebar.Sectie.INGERICHTE_TALENTEN, "Ingerichte talenten beheren");

        periodeFilterComboBox = new ComboBox<>();
        periodeFilterComboBox.setPrefWidth(260);
        periodeFilterComboBox.setConverter(periodeConverter());

        nieuwIngerichtTalentButton = new Button("+ Nieuw ingericht talent");
        nieuwIngerichtTalentButton.getStyleClass().add("primary-button");

        ingerichteTalentenTable = maakTabel();

        formulierTitelLabel = new Label("Nieuw ingericht talent");
        ...
        periodeComboBox.setConverter(periodeConverter());

        talentComboBox = new ComboBox<>();
        talentComboBox.setMaxWidth(Double.MAX_VALUE);
        ...
        doelgroepComboBox.setConverter(doelgroepConverter());

        naamField = new TextField();
        naamField.setPromptText("Bijv. Schaken observatie");

        omschrijvingArea = new TextArea();
        omschrijvingArea.setPromptText("Periode-specifieke omschrijving");
        omschrijvingArea.setWrapText(true);
        omschrijvingArea.setPrefRowCount(4);

        maxCapaciteitSpinner = new Spinner<>(1, 200, 10);
        maxCapaciteitSpinner.setEditable(true);
        maxCapaciteitSpinner.setMaxWidth(Double.MAX_VALUE);

        actiefCheckBox = new CheckBox("Actief voor nieuwe voorkeuren en verdelingen");
        actiefCheckBox.setSelected(true);

        gekoppeldeLeerkrachtenList = new ListView<>();
        gekoppeldeLeerkrachtenList.setPrefHeight(108);
        gekoppeldeLeerkrachtenList.setPlaceholder(new Label("Nog geen leerkracht gekoppeld."));
        gekoppeldeLeerkrachtenList.setCellFactory(list -> leerkrachtCell());

        leerkrachtComboBox = new ComboBox<>();
        leerkrachtComboBox.setPromptText("Kies een leerkracht");
        leerkrachtComboBox.setMaxWidth(Double.MAX_VALUE);
        leerkrachtComboBox.setConverter(leerkrachtConverter());

        leerkrachtToevoegenButton = new Button("Toevoegen");
        leerkrachtToevoegenButton.getStyleClass().add("primary-outline-button");

        leerkrachtVerwijderenButton = new Button("Verwijderen");
        leerkrachtVerwijderenButton.getStyleClass().add("secondary-button");
        ...
        statusLabel = new Label("Selecteer een periode om ingerichte talenten te beheren.");
    ...
    private TableView<IngerichtTalent> maakTabel() {
        TableView<IngerichtTalent> table = new TableView<>();
        table.setPlaceholder(new Label("Geen ingerichte talenten voor deze periode."));
        table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY_FLEX_LAST_COLUMN);

        TableColumn<IngerichtTalent, String> naamKolom = new TableColumn<>("Naam");
        naamKolom.setCellValueFactory(data -> new ReadOnlyStringWrapper(data.getValue().getNaam()));
        naamKolom.setPrefWidth(180);

        TableColumn<IngerichtTalent, String> talentKolom = new TableColumn<>("Talent");
        talentKolom.setCellValueFactory(data -> new ReadOnlyStringWrapper(data.getValue().getTalent().getNaam()));
        talentKolom.setPrefWidth(125);

        TableColumn<IngerichtTalent, String> doelgroepKolom = new TableColumn<>("Doelgroep");
        doelgroepKolom.setCellValueFactory(data -> new ReadOnlyStringWrapper(formatteerDoelgroepKort(data.getValue().getDoelgroep())));
        doelgroepKolom.setPrefWidth(150);

        TableColumn<IngerichtTalent, String> capaciteitKolom = new TableColumn<>("Cap.");
        capaciteitKolom.setCellValueFactory(data -> new ReadOnlyStringWrapper(String.valueOf(data.getValue().getMaxCapaciteit())));
        capaciteitKolom.setPrefWidth(70);

        TableColumn<IngerichtTalent, String> leerkrachtenKolom = new TableColumn<>("Leerkrachten");
        leerkrachtenKolom.setCellValueFactory(data -> new ReadOnlyStringWrapper(formatteerLeerkrachten(data.getValue().getLeerkrachten())));
        leerkrachtenKolom.setPrefWidth(190);

        TableColumn<IngerichtTalent, String> statusKolom = new TableColumn<>("Status");
        statusKolom.setCellValueFactory(data -> new ReadOnlyStringWrapper(data.getValue().isActief() ? "Actief" : "Inactief"));
        statusKolom.setPrefWidth(90);

        table.getColumns().addAll(naamKolom, talentKolom, doelgroepKolom, capaciteitKolom, leerkrachtenKolom, statusKolom);
        return table;
    }
    ...
    private VBox maakInhoud() {
        Label titel = new Label("Ingerichte talenten beheren");
        titel.getStyleClass().add("page-title");

        Label ondertitel = new Label("Configureer talenten per periode, doelgroep en capaciteit en koppel maximaal twee leerkrachten.");
        ondertitel.getStyleClass().add("page-subtitle");

        Label filterLabel = new Label("Periode");
        filterLabel.getStyleClass().add("field-label");
        VBox filterBox = new VBox(5, filterLabel, periodeFilterComboBox);

        Region headerSpacer = new Region();
        HBox.setHgrow(headerSpacer, Priority.ALWAYS);

        HBox header = new HBox(18, new VBox(4, titel, ondertitel), headerSpacer, filterBox, nieuwIngerichtTalentButton);
        header.setAlignment(Pos.BOTTOM_LEFT);

        Label overzichtTitel = new Label("Ingerichte talenten");
        overzichtTitel.getStyleClass().add("section-heading");

        Label overzichtUitleg = new Label("Selecteer een ingericht talent om naam, omschrijving, capaciteit, status of leerkrachten te wijzigen.");
        overzichtUitleg.setWrapText(true);
        overzichtUitleg.getStyleClass().add("muted-label");

        VBox overzichtCard = new VBox(10, overzichtTitel, overzichtUitleg, ingerichteTalentenTable);
        overzichtCard.getStyleClass().add("content-card");
        overzichtCard.setPadding(new Insets(22));
        overzichtCard.setMinWidth(610);
        HBox.setHgrow(overzichtCard, Priority.ALWAYS);
        VBox.setVgrow(ingerichteTalentenTable, Priority.ALWAYS);
    ...
    private VBox maakFormulierCard() {
        Label periodeLabel = maakFieldLabel("Talentenperiode");
        Label talentLabel = maakFieldLabel("Basistalent");
        Label doelgroepLabel = maakFieldLabel("Doelgroep");
        Label naamLabel = maakFieldLabel("Naam");
        Label omschrijvingLabel = maakFieldLabel("Omschrijving");
        Label capaciteitLabel = maakFieldLabel("Maximumcapaciteit");
        Label leerkrachtenLabel = maakFieldLabel("Leerkrachten (0–2)");

        HBox leerkrachtToevoegenBox = new HBox(8, leerkrachtComboBox, leerkrachtToevoegenButton);
        HBox.setHgrow(leerkrachtComboBox, Priority.ALWAYS);

        HBox leerkrachtActies = new HBox(leerkrachtVerwijderenButton);
        leerkrachtActies.setAlignment(Pos.CENTER_RIGHT);

        HBox knoppen = new HBox(10, annulerenButton, opslaanButton);
        knoppen.setAlignment(Pos.CENTER_RIGHT);

        VBox formulier = new VBox(
                8,
                formulierTitelLabel,
                periodeLabel,
                periodeComboBox,
        ...
        formulier.getStyleClass().add("content-card");
        formulier.setPadding(new Insets(22));
        formulier.setPrefWidth(420);
        formulier.setMinWidth(390);
        return formulier;
    }
    ...
    public void setPeriodes(List<TalentenPeriode> periodes) {
        periodeFilterComboBox.setItems(FXCollections.observableArrayList(periodes));
        periodeComboBox.setItems(FXCollections.observableArrayList(periodes));
    }

    public void setTalenten(List<Talent> talenten) {
        talentComboBox.setItems(FXCollections.observableArrayList(talenten));
    }
    ...
    public void setIngerichteTalenten(List<IngerichtTalent> ingerichteTalenten) {
        ingerichteTalentenTable.getItems().setAll(ingerichteTalenten);
        ingerichteTalentenTable.refresh();
    }

    public void selecteerFilterPeriode(TalentenPeriode periode) {
        periodeFilterComboBox.getSelectionModel().select(periode);
    }

    public void toonNieuwFormulier(TalentenPeriode periode) {
        formulierTitelLabel.setText("Nieuw ingericht talent");
        ingerichteTalentenTable.getSelectionModel().clearSelection();

        periodeComboBox.setDisable(false);
        talentComboBox.setDisable(false);
        doelgroepComboBox.setDisable(false);
        actiefCheckBox.setDisable(true);

        periodeComboBox.getSelectionModel().select(periode);
        talentComboBox.getSelectionModel().clearSelection();
    ...
    public void toonWijzigFormulier(IngerichtTalent ingerichtTalent) {
        formulierTitelLabel.setText("Ingericht talent wijzigen");

        periodeComboBox.getSelectionModel().select(ingerichtTalent.getTalentenPeriode());
        talentComboBox.getSelectionModel().select(ingerichtTalent.getTalent());
        doelgroepComboBox.getSelectionModel().select(ingerichtTalent.getDoelgroep());
        naamField.setText(ingerichtTalent.getNaam());
        omschrijvingArea.setText(ingerichtTalent.getOmschrijving());
        ...
        doelgroepComboBox.setDisable(true);
        actiefCheckBox.setDisable(false);

        setStatus("Basistalent, periode en doelgroep blijven bij een bestaand ingericht talent ongewijzigd.");
    }
    ...
    public boolean voegLeerkrachtAanFormulierToe(Leerkracht leerkracht) {
        ...
        if (gekoppeldeLeerkrachtenList.getItems().contains(leerkracht)) {
            setStatus("Deze leerkracht is al gekoppeld.");
            return false;
        }
        if (gekoppeldeLeerkrachtenList.getItems().size() >= 2) {
            toonFout("Een ingericht talent mag maximaal twee leerkrachten hebben.");
            return false;
        }
    ...
    public void verwijderGeselecteerdeLeerkrachtUitFormulier() {
        Leerkracht geselecteerd = gekoppeldeLeerkrachtenList.getSelectionModel().getSelectedItem();
        if (geselecteerd == null) {
            setStatus("Selecteer eerst een gekoppelde leerkracht om te verwijderen.");
            return;
        }
    ...
    public List<Leerkracht> getFormulierLeerkrachten() {
        return List.copyOf(gekoppeldeLeerkrachtenList.getItems());
    }
    ...
    private String formatteerDoelgroepKort(Doelgroep doelgroep) {
        ...
        return switch (doelgroep) {
            case OBSERVATIE_OPLEIDINGSFASE_EERSTEGRAAD_AB -> "Observatie / opleiding";
            case KWALIFICATIEFASE_TWEEDEGRAAD_AB -> "Kwalificatie";
        };
    }
    ...
    private StringConverter<TalentenPeriode> periodeConverter() {
        return new StringConverter<>() {
            ...
            public String toString(TalentenPeriode periode) {
                ...
                return periode.getSchooljaar().getNaam() + " • " + periode.getNaam();
            }
    ...
    private ListCell<Leerkracht> leerkrachtCell() {
        return new ListCell<>() {
            @Override
            protected void updateItem(Leerkracht leerkracht, boolean empty) {
                super.updateItem(leerkracht, empty);
                setText(empty || leerkracht == null ? null : leerkracht.getVoornaam() + " " + leerkracht.getAchternaam());
            }
        };
    }
