...
class VoorkeurenImportVerdelingIntegratieTest {
    ...
    void setUp() throws SQLException {
        ...
        probleemRepository = new PostgresVoorkeurImportProbleemRepository();
    }

    @Test
    void gecorrigeerdeHerimportWordtOpgeslagenEnLeerlingKanDaarnaAutomatischVerdeeldWorden(@TempDir Path tempDir) throws IOException {
        // ARRANGE
        ...
        Leerling sofie = leerlingRepository.save(
                new Leerling(
                        "Sofie",
                        "Peeters",
                        klas
        ...
        IngerichtTalent kokenHerfst = ingerichtTalentRepository.save(
                new IngerichtTalent(
                        koken,
                        periode,
                        "Koken basis",
        ...
        AutomatischeVerdelingService verdelingService = new AutomatischeVerdelingService(
                voorkeurRepository,
        ...
        Path onvolledigBestand = tempDir.resolve("voorkeuren_onvolledig.xlsx");
        ...
        // ACT 1 - eerste onvolledige import
        ...
        List<Voorkeur> voorkeurenNaEersteImport =
                voorkeurRepository.zoekVoorLeerlingEnPeriode(sofie, periode);

        List<VoorkeurImportProbleem> problemenNaEersteImport =
                probleemRepository.zoekVoorLeerlingEnPeriode(sofie, periode);

        assertEquals(2, voorkeurenNaEersteImport.size());

        assertEquals(1, problemenNaEersteImport.size());
        assertEquals(
                "Keuze 3 is niet ingevuld",
                problemenNaEersteImport.getFirst().getReden()
        );

        // ARRANGE 2 - gecorrigeerd bestand
        Path gecorrigeerdBestand = tempDir.resolve("voorkeuren_gecorrigeerd.xlsx");
        ...
        assertEquals(1, opgeslagenToewijzingen.size());

        assertEquals(
                sofie.getId(),
                opgeslagenToewijzingen.getFirst().getLeerling().getId()
        );

        assertEquals(
                schakenHerfst.getId(),
                opgeslagenToewijzingen.getFirst().getIngerichtTalent().getId()
        );
    }

    private void vulKeuzesIn(Path bestand, String sheetNaam, String keuze1, String keuze2, String keuze3) throws IOException {
        ...
        try (workbook; OutputStream outputStream = Files.newOutputStream(bestand)) {
            Sheet sheet = workbook.getSheet(sheetNaam);
            Row leerlingRij = sheet.getRow(1);

            leerlingRij.getCell(2).setCellValue(keuze1);
            leerlingRij.getCell(3).setCellValue(keuze2);

            if (keuze3 == null) {
                leerlingRij.getCell(4).setBlank();
            } else {
                leerlingRij.getCell(4).setCellValue(keuze3);
            }
