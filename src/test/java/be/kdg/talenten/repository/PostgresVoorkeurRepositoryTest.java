...
public class PostgresVoorkeurRepositoryTest {
    ...
    void zoekVoorPeriodeGeeftAlleenVoorkeurenVanGevraagdePeriodeTerug() {
        ...
        Leerkracht tom = leerkrachtRepository.save(new Leerkracht("Tom", "Laforce"));

        IngerichtTalent schakenHerfst = ingerichtTalentRepository.save(new IngerichtTalent(schaken, herfst, 10, Doelgroep.OBSERVATIE_OPLEIDINGSFASE_EERSTEGRAAD_AB, List.of(tom)));
        IngerichtTalent voetbalHerfst = ingerichtTalentRepository.save(new IngerichtTalent(voetbal, herfst, 10, Doelgroep.OBSERVATIE_OPLEIDINGSFASE_EERSTEGRAAD_AB, List.of(tom)));
        IngerichtTalent schakenWinter = ingerichtTalentRepository.save(new IngerichtTalent(schaken, winter, 10, Doelgroep.OBSERVATIE_OPLEIDINGSFASE_EERSTEGRAAD_AB, List.of(tom)));

        Voorkeur opgeslagenSchakenHerfst = voorkeurRepository.save(new Voorkeur(tim, herfst, schakenHerfst, 1));
        Voorkeur opgeslagenVoetbalHerfst = voorkeurRepository.save(new Voorkeur(tim, herfst, voetbalHerfst, 2));
        voorkeurRepository.save(new Voorkeur(tim, winter, schakenWinter, 1));

        // ACT
        List<Voorkeur> resultaat = voorkeurRepository.zoekVoorPeriode(herfst);

        // ASSERT
        Assertions.assertEquals(2, resultaat.size());

        Voorkeur eersteVoorkeur = resultaat.getFirst();
        Voorkeur tweedeVoorkeur = resultaat.get(1);

        Assertions.assertEquals(opgeslagenSchakenHerfst.getId(), eersteVoorkeur.getId());
        Assertions.assertEquals(1, eersteVoorkeur.getVoorkeurNummer());
        Assertions.assertEquals(tim.getId(), eersteVoorkeur.getLeerling().getId());
        Assertions.assertEquals(herfst.getId(), eersteVoorkeur.getTalentenPeriode().getId());
        Assertions.assertEquals(schakenHerfst.getId(), eersteVoorkeur.getIngerichtTalent().getId());

        Assertions.assertEquals(opgeslagenVoetbalHerfst.getId(), tweedeVoorkeur.getId());
        Assertions.assertEquals(2, tweedeVoorkeur.getVoorkeurNummer());
        Assertions.assertEquals(tim.getId(), tweedeVoorkeur.getLeerling().getId());
        Assertions.assertEquals(herfst.getId(), tweedeVoorkeur.getTalentenPeriode().getId());
        Assertions.assertEquals(voetbalHerfst.getId(), tweedeVoorkeur.getIngerichtTalent().getId());

        Assertions.assertTrue(resultaat.stream().allMatch(voorkeur -> voorkeur.getTalentenPeriode().getId().equals(herfst.getId())));
        Assertions.assertFalse(resultaat.stream().anyMatch(voorkeur -> voorkeur.getIngerichtTalent().getId().equals(schakenWinter.getId())));
    }
